...
if st.button("Bereken"):
    ...
    # Bepaling type auto
    marge_auto = st.radio("Type auto:", ["BTW-auto (2,7% bijtellingcorrectie)", "Marge-auto (1,5%)"]) == "Marge-auto (1,5%)"
    btw_correctie_percentage = 0.015 if marge_auto else 0.027

    # Brandstof exclusief btw voor zakelijk
    brandstof_btw = brandstofprijs * (21 / 121)
    brandstofprijs_ex_btw = brandstofprijs - brandstof_btw
    brandstof_kosten_ex_btw = (verbruik_l_per_100km / 100) * (zakelijk_km_per_jaar + prive_km_per_jaar) * brandstofprijs_ex_btw

    # BTW-correctie bijtelling op basis van type auto
    bijtelling_correctie = cataloguswaarde * btw_correctie_percentage
    bijtelling_netto += bijtelling_correctie
    ...
    prive_totaal_kosten = aanschafprijs - restwaarde + jaarlijkse_kosten_ex_btw * gebruiksjaren - vergoeding_per_jaar * gebruiksjaren + vergoeding_bv_last_na_vpb * gebruiksjaren
    zakelijk_totaal_kosten = afschrijving * gebruiksjaren + jaarlijkse_kosten_ex_btw * gebruiksjaren + bijtelling_netto * gebruiksjaren - btw_terug
    ...
    brandstof_per_jaar = brandstof_kosten_ex_btw
    ...
    for jaar in jaren:
        ...
        st.write(f"- Brandstofkosten (excl. btw): €{round(brandstof_per_jaar, 2)}")
        ...
        st.write(f"👉 **Totaal privé dat jaar:** €{round(aanschaf_prive_jaar + brandstof_per_jaar + overige_per_jaar - vergoeding_per_jaar + vergoeding_na_vpb_jaar, 2)}")

        st.write("**Zakelijk**:")
        st.write(f"- Afschrijving: €{round(afschrijving_per_jaar, 2)}")
        st.write(f"- Brandstofkosten (excl. btw): €{round(brandstof_per_jaar, 2)}")
        st.write(f"- Overige kosten: €{round(overige_per_jaar, 2)}")
        st.write(f"- Bijtelling (incl. correctie): €{round(bijtelling_per_jaar, 2)}")
        st.write(f"- Btw-teruggave (aftrek): -€{round(btw_terug_jaar, 2)}")
        st.write(f"👉 **Totaal zakelijk dat jaar:** €{round(afschrijving_per_jaar + brandstof_per_jaar + overige_per_jaar + bijtelling_per_jaar - btw_terug_jaar, 2)}")
