...
# Invoer
st.subheader('🔧 Instellingen vooraf')
marge_auto = st.radio("Type auto:", ["BTW-auto (2,7% bijtellingcorrectie)", "Marge-auto (1,5%)"], index=0) == "Marge-auto (1,5%)"
...
if st.button("Bereken"):
    ...
    # Btw-correctie los toegepast
    btw_correctie = cataloguswaarde * btw_correctie_percentage

    # Bijtelling zonder btw-correctie
    ...
    zakelijk_totaal_kosten = afschrijving * gebruiksjaren + jaarlijkse_kosten_ex_btw * gebruiksjaren + bijtelling_per_jaar * gebruiksjaren + btw_correctie * gebruiksjaren
    ...
    # VPB-voordeel voor zakelijke kosten
    zakelijke_kosten_voor_vpb = afschrijving + brandstof_kosten_ex_btw + overige_kosten + bijtelling_per_jaar + btw_correctie
    vpb_voordeel_jaar = zakelijke_kosten_voor_vpb * (vpb_percentage / 100)
    zakelijk_totaal_kosten -= vpb_voordeel_jaar * gebruiksjaren

    st.subheader("📊 Resultaten (incl. VPB-voordeel)")
    ...
    for jaar in jaren:
        ...
        st.write(f"- Bijtelling (netto): €{round(bijtelling_per_jaar, 2)}")
        st.write(f"- Btw-correctie: €{round(btw_correctie_jaar, 2)}")
        st.write(f"- VPB-voordeel: -€{round(vpb_voordeel_jaar, 2)}")
        st.write(f"👉 **Totaal zakelijk dat jaar:** €{round(afschrijving_per_jaar + brandstof_zakelijk_per_jaar + overige_per_jaar + bijtelling_per_jaar + btw_correctie_jaar - vpb_voordeel_jaar, 2)}")
