...
if st.button("Bereken"):
    ...
    for jaar in jaren:
        ...
        st.write(f"- Brandstofkosten: €{round(brandstof_per_jaar, 2)}")
        st.write(f"- Overige kosten: €{round(overige_per_jaar, 2)}")
        st.write(f"- Kilometervergoeding (aftrek): -€{round(vergoeding_per_jaar, 2)}")
        st.write(f"👉 **Totaal privé dat jaar:** €{round(aanschaf_prive_jaar + brandstof_per_jaar + overige_per_jaar - vergoeding_per_jaar, 2)}")
